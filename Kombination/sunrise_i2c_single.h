...
const int WAIT_FOR_PIN_MS  = 6000;
...
void read_sensor_measurements( uint8_t target )
{
   ...
   byteHi = Wire.read();
   byteLo = Wire.read();
   uint16_t co2Val_filtered = ( (int16_t)(int8_t)byteHi << 8 ) | (uint16_t)byteLo;

   byteHi = Wire.read();
   byteLo = Wire.read();
   uint16_t co2Val_unfiltered = ( (int16_t)(int8_t)byteHi << 8 ) | (uint16_t)byteLo;

   co2Val = co2Val_filtered;
   ...
   if ( sunriseOutput )
   {
      ...
      Serial.print( co2Val_filtered );
      Serial.println( " ppm" );

      Serial.print( "CO2 (unfiltered): " );
      Serial.print( co2Val_unfiltered );
      Serial.println( " ppm" );
